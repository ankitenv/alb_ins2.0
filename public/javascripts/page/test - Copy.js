var	urlData = getUrlVars();
	gameLevelURL = 'getTestLevel/?i='+urlData['i']+'&p='+urlData['p'],
	gls ={},
	counter = 0,
	CURR_LEVEL = -1,
	CURR_TARGET = -1,
...
function sendPlayerInfotoServer(){
	data = {t:urlData["i"], p:urlData["p"]};
	socket.emit("addPlayertoTest", data);
}

var p=setInterval(function(){
					data = {c:urlData["i"]};
					socket.emit("updateTestPlayerList", data);
				},
				2000);
				
var b=setInterval(function(){
					$(htmlObj.player_container).html(playerList);
				},
				1000);

//loading
function loadGameInfo(){
	var url="getTestInfo/?i="+urlData["i"];
	$.ajax({
		url: url,
		type: "GET",
		dataType: 'json',
		...
		success: function(data) {
			var obj=data[0];
			$(htmlObj.header).html(obj.test_name);
			if(obj.limited_time_target==1){
				LIMIT_TIME_TARGET=true;
				GAME_TIME=parseInt(obj.time_allowed);
			}
		},
		error  : function(){
	...
	$.ajax({
		...
		success: function(data) {
			for(var i=0;i<data.length;i++){
				...
				TOTAL_GAME_TARGETS+=parseInt(data[i].total_targets);
				counter++;
			}
...
function lifeUpdateTimeBasis(){
	if(LIFE==0){
		$(htmlObj.status).html("Sorry! Test is Over..");
		socket.emit('setTestOverPlayers', {i:urlData['i'],p:urlData['p']});
...
function loadPlayerInfo(id, comp_id){
	var playerURL='getTestPlayerInfo/?id='+id;
	$.ajax({
		url: playerURL,
		type: "GET",
		dataType: 'json',
...
function loadGameLevelFunctions(id){
	var levelFunctionURL='getTestLevelFunctions/?id='+id;
	$.ajax({
		url: levelFunctionURL,
		type: "GET",
		dataType: 'json',
		async:false,
		success: function(data) {
			...
			gls[counter].functions = functions;
		},
		error  : function(){
			$(htmlObj.status).html('error'); 
		}
...
function defineGameLevelTargets(){
	try{
		...
		$(htmlObj.status).html("Some Problem occurs. Please click Refresh button(at Top-Right-Corner) above to reload Test.");
...
function CompleteGame(){
	$(htmlObj.result_status).addClass("Compl");
	$(htmlObj.result_status).html("Congrats you have finished the Competition!");
}
...
$(document).ready(function(){
	$(htmlObj.status).html("Please wait while competition is loading......");
	socket.emit("checkPlayerPlayTest", {i:urlData['i'], p:urlData['p']});
	...
	$(htmlObj.player_btn).click(function(){
		$(htmlObj.player_container).show();
		setTimeout(function() {
			$(htmlObj.player_container).hide();
		}, 5000);
	});
	$(htmlObj.submit).click(function(){
		...
		if(LIFE>0){
			if(target===result){
				updateScore();
				var score_data={t:urlData['i'], p:urlData['p'], s:score};
				socket.emit("updateTestPlayerScore",score_data);
				if(CURR_LEVEL===(TOTAL_LEVEL-1) && CURR_TARGET===(parseInt(gls[CURR_LEVEL]["total_targets"])-1)){
					CompleteGame();					
					socket.emit("setTestFinishedPlayers",{p:urlData['p'],i:urlData['i']});
				}
				else{
					CorrectAnswer();
					updateGame();
				...
				LIFE--;
				if(LIFE===0){
					$(htmlObj.status).html("Sorry! Competition is Over..");
					socket.emit('setTestOverPlayers', {i:urlData['i'],p:urlData['p']});
					window.location.href=window.CONFIG.URL;
				}
				else{
