var	urlData = getUrlVars();
	...
	gvar = {
		...
		URL : {
			...
			getTestInfo : 'getTestInfo/?i='+urlData['i'],
			levelCardURL : function(id){
							return 'getTestLevelCards/?id='+id;
						},
			levelFunctionURL : function(id){
							return 'getTestLevelFunctions/?id='+id;
						},
			playerURL : function(id){
							return 'getTestPlayerInfo/?id='+id;
						},
...
var p=setInterval(function(){
					data = {c:urlData["i"]};
					socket.emit("getTestPlayerList",data);
...
function loadGameInfo(){
	$.ajax({
		url: gvar.URL.getTestInfo,
		type: "GET",
		dataType: 'json',
		async:false,
		success: function(data) {
			var obj=data[0];
			$(gvar.HTML.header).html(obj.test_name);
			if(obj.limited_time_target==1){
				gvar.LIMIT_TIME_TARGET=true;
				gvar.GAME_TIME=parseInt(obj.time_allowed);
			}
		},
		error  : function(){
...
function defineGameLevelTargets(){
	try{
		...
		if(gvar.LIMIT_TIME_TARGET){
			gvar.TARGET_TIME=parseInt((gvar.GAME_TIME * 60)/gvar.TOTAL_GAME_TARGETS);
...
$(document).ready(function(){
	$(gvar.HTML.status).html("Please wait while test is loading......");
	...
	$(gvar.HTML.submit).click(function(){
		...
		if(gvar.LIFE>0){
			if(target===result){
				updateScore();
				var score_data={t:urlData['i'], p:urlData['p'], s:gvar.SCORE};
				//socket.emit("updateTestPlayerScore",score_data);
				socket.emit("wowfunction",score_data);
				...
				gvar.LIFE--;
				if(gvar.LIFE===0){
					$(gvar.HTML.status).html("Sorry! Test is Over..");
					socket.emit('setTestOverPlayers', {i:urlData['i'],p:urlData['p']});
					window.location.href=window.CONFIG.URL;
				}
				else{
